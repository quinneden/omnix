{
  ...
  in {
    ...
    #   Extensions.Install =
    #     with config.home-manager.users.quinn.programs.firefox.package
    #     [
    ...
    programs.waybar = {
      ...
      in {
        ...
        cava = {
          ...
          # cava_config = "${config.users.users.quinn.home}/.config/cava/config";
        ...
        mpris = {
          ...
          min-length = 33;
          max-length = 33;
          format = with rose-pine; "{status_icon}  <span foreground='#${iris}'>{artist}</span> ࢇ <span foreground='#${foam}'> {title}</span>";
          dynamic-len = 30;
